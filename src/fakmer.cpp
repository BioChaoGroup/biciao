...
std::pair<std::string, std::string> getBarcode(std::string& line)
{
    ...
    if (read_type.compare("other")){
        read_name = line.substr(0, line.find_first_of(" \r\t\n"));
        barcode = read_name;
    }
    else if (!read_type.compare("stLFR")){
...
int main(int argc, char* argv[])
{
    ...
    outputf << header;  // Write the header to the output file
    outputf << std::endl;
    ...
    while (getline(readsf, line))
    {
        if (line[0] == '>')
        {
            ...
            barcode = line.substr(1); // Use sequence ID as barcode
